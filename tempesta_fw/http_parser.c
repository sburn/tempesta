		   ...
		   unsigned int *parsed)
{
	...
	*parsed = 0;
	...
	TFW_HTTP_PARSE_SPECHDR_VAL(Req_HdrX_Forwarded_ForV, msg,
...
tfw_http_parse_terminate(TfwHttpMsg *hm)
{
	...
	if (!(digs = tfw_ultoa(hm->content_length, c_len, TFW_ULTOA_BUF_SIZ)))
		return false;

	return !tfw_http_msg_hdr_xfrm(hm, "Content-Length",
				      sizeof("Content-Length") - 1, c_len,
				      digs, TFW_HTTP_HDR_CONTENT_LENGTH, 0);
}
		    ...
		    unsigned int *parsed)
{
	...
	*parsed = 0;
	...
	__FSM_TX_AF_OWS(Resp_HdrCache_Control, ':', Resp_HdrCache_CtrlV, RGen_HdrOther);
